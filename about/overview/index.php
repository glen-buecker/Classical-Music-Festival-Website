...
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
  ...
  <!-- Main Theme Stylesheet -->
	...
	<!-- Plugin injected code -->
		...
		<link rel='stylesheet' type='text/css' media='all' href='files/stacks_page_page2.css?rwcache=685984326' />
        <script type='text/javascript' charset='utf-8' src='https://ajax.googleapis.com/ajax/libs/jquery/2.2.4/jquery.min.js'></script>
        
        <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css">
        
		
		<script type='text/javascript' charset='utf-8' src='files/stacks_page_page2.js?rwcache=685984326'></script>
...
//include_once "files/stacks_page_page2.php";
...
<div class="margins">
	<div id='stacks_out_9' class='stacks_out'><div id='stacks_in_9' class='stacks_in com_elixir_stacks_foundryHeader_stack'>

<div class="text-xs-left">
	
	<h2 class="theme_style  ">The Festival</h2>
	
</div>
</div></div><div id='stacks_out_8' class='stacks_out'><div id='stacks_in_8' class='stacks_in com_nimblehost_stack_armadilloSoloContent_stack'>
...
if ( $setupIsComplete ) {
    ...
    $soloContent = displaySoloContent( $dbLink, '4', 'true', $armadilloOptions );
    ...
    if ( $soloContent == Armadillo_Language::public_msg($armadilloOptions, 'ARM_CONTENT_SELECTED_ITEM_MISSING') ) {
	    ...
	    if ( $soloContent['publish'] ) {
	    	$padding_for_4 = $soloContent['content'] == '' ? ' armadilloPadding' : '';
	    	echo '<div id="armadilloSoloContent_4_stacks_in_8" class="armadilloContent' . $padding_for_4 . '">' . $soloContent['content'] . '</div>';
	    } else {
	    	echo '<div id="armadilloSoloContent_4_stacks_in_8" class="armadilloContent draft"></div>';
    	}
    }
} else {
...
?>
	<!--<style>#armadilloSoloContent_4_stacks_in_8.draft { min-height: 2em; }</style>-->
	...
	<?php else: ?>
	<div class="armadilloEditButton"><a href="#editSoloContentID_4_stacks_in_8" class="displayEditor"><i class="fa fa-pencil-square-o"></i>&nbsp;ID: 4</a></div>
	<?php endif; ?>
	<div id="editSoloContentID_4_stacks_in_8" class="editSoloContent" style="display:none;">
		<div id="editSoloContentID_4_stacks_in_8_toolbar"></div>
        ...
        <!-- Rich Text Editor -->
        <textarea id="soloContentID_4_stacks_in_8" name="soloContentID_4_stacks_in_8"><?php echo htmlspecialchars($soloContent['content'], ENT_QUOTES, 'UTF-8'); ?></textarea>
        <?php else: ?>
        <!-- Markdown Editor -->
        <textarea autofocus id="soloContentID_4_stacks_in_8" name="soloContentID_4_stacks_in_8" data-uk-htmleditor="{markdown:true,autofocus:true}"><?php if ( isset($soloContent['format']) and $soloContent['format'] == 'markdown' ) { echo $soloContent['rawContent']; } ?></textarea>
        <?php if ( !isset($soloContent['format']) or $soloContent['format'] == 'html' ): ?>
        <script>document.addEventListener("DOMContentLoaded", function(event) { soloContentID_4_stacks_in_8 = document.getElementById('soloContentID_4_stacks_in_8'); soloContentID_4_stacks_in_8.value = toMarkdown('<?php echo str_replace(array("\n", "\r"),'', str_replace("'","\'",$soloContent['rawContent'])); ?>'); });</script>
        <?php endif; ?>
	    <?php endif; ?>
		<p class="saveSoloContentDetails">
			<a href="#soloContentID_4_stacks_in_8" id="saveSoloContentID_4_stacks_in_8" class="saveSoloContentButton armadilloButton"><?php echo Armadillo_Language::public_msg($armadilloOptions, 'ARM_SOLO_CONTENT_SAVE_TEXT'); ?></a>
	...
	<script>
		document.addEventListener("DOMContentLoaded", function(event) {
			var armadilloSoloContent_4_stacks_in_8 = {};
			armadilloSoloContent_4_stacks_in_8 = (function(){
				<?php if ( !isset($_COOKIE['armadillo']['editorType']) or ( isset($_COOKIE['armadillo']['editorType']) and ( $_COOKIE['armadillo']['editorType'] == 'richtext' or $_COOKIE['armadillo']['editorType'] == 'basictext' ) ) ): ?>
				$R('#soloContentID_4_stacks_in_8', {
					toolbarExternal: '#editSoloContentID_4_stacks_in_8_toolbar',
					toolbarFixed: true,
					autoresize: false,
					maxHeight: 500,
				...
				<?php endif; ?>
				armasolo.jQuery('#saveSoloContentID_4_stacks_in_8').click(function(){
					<?php if ( !isset($_COOKIE['armadillo']['editorType']) or ( isset($_COOKIE['armadillo']['editorType']) and ( $_COOKIE['armadillo']['editorType'] == 'richtext' or $_COOKIE['armadillo']['editorType'] == 'basictext' ) ) ): ?>
					var updatedContent = $R('#soloContentID_4_stacks_in_8', 'source.getCode');
					var previewHTML = updatedContent;
					updatedContent = encodeURIComponent(updatedContent);
					var format = 'html';
					<?php else: ?>
					var updatedContent = encodeURIComponent(armasolo.jQuery('#soloContentID_4_stacks_in_8').val());
					var previewHTML = armasolo.jQuery('#editSoloContentID_4_stacks_in_8 .uk-htmleditor .uk-htmleditor-content .uk-htmleditor-preview').html();
					var format = 'markdown';
					<?php endif; ?>
					armasolo.jQuery.ajax({
						beforeSend: function(){ armasolo.jQuery('#editSoloContentID_4_stacks_in_8 .saveSoloContent.result i.fa').removeClass('fa-times, fa-check').parent().css('opacity',0); armasolo.jQuery('#editSoloContentID_4_stacks_in_8 .saveSoloContent.progress').fadeTo(300,1); },
						url: '../../rw_common/plugins/stacks/armadillo/core/model/updateArmadilloSettings.php',
						type: 'post',
						data: 'action=updateContent&format=' + format + '&contentID=4&content=' + updatedContent,
						success: function(data, status) {
							if ( data == 'true' ) {
								armasolo.jQuery('#editSoloContentID_4_stacks_in_8 .saveSoloContent.result i.fa').addClass('fa-check').parent().fadeTo(300,1);
								armasolo.jQuery('#armadilloSoloContent_4_stacks_in_8').html(previewHTML);
							} else { console.log(data); console.log(status); armasolo.jQuery('#editSoloContentID_4_stacks_in_8 .saveSoloContent.result i.fa').addClass('fa-times').parent().fadeTo(300,1); }
						},
						error: function(jqxhr, status, error) { console.log(error); console.log(jqxhr); armasolo.jQuery('#editSoloContentID_4_stacks_in_8 .saveSoloContent.result i.fa').addClass('fa-times').parent().fadeTo(300,1); },
						complete: function(){ armasolo.jQuery('#editSoloContentID_4_stacks_in_8 .saveSoloContent.progress').fadeTo(300,0); }
					});
				});
			})(armadilloSoloContent_4_stacks_in_8);
		});
	</script>
<?php endif; ?>
</div>

</div></div><div id='stacks_out_12' class='stacks_out'><div id='stacks_in_12' class='stacks_in '><div id='stacks_out_12_933' class='stacks_out'><div id='stacks_in_12_933' class='stacks_in stack_stack'><div id='stacks_out_12_226' class='stacks_out'><div id='stacks_in_12_226' class='stacks_in html_stack'><a class="social-facebook social-import" href=http://www.facebook.com/classicalmusicfestival></a></div></div><div id='stacks_out_12_937' class='stacks_out'><div id='stacks_in_12_937' class='stacks_in com_nimblehost_stack_armadilloSoloContent_stack'>
<div class="armadilloSoloContentContainer">
<?php
/* Only for use with the Armadillo CMS from NimbleHost*/
$siteAssetPath = '../../rw_common/plugins/stacks';
...
if ( $setupIsComplete ) {
    ...
    if ( $soloContent == Armadillo_Language::public_msg($armadilloOptions, 'ARM_CONTENT_SELECTED_ITEM_MISSING') ) {
	    ...
	    if ( $soloContent['publish'] ) {
	    	...
	    	echo '<div id="armadilloSoloContent_6_stacks_in_12_937" class="armadilloContent' . $padding_for_6 . '">' . $soloContent['content'] . '</div>';
	    } else {
	    	echo '<div id="armadilloSoloContent_6_stacks_in_12_937" class="armadilloContent draft"></div>';
    	}
    }
} else {
	// Setup isn't complete so display a notification
...
<?php 
	...
	if ( 
		...
		and ( $_COOKIE['armadillo']['role'] == 'admin' or $_COOKIE['armadillo']['role'] == 'editor' or $_COOKIE['armadillo']['userID'] == $soloContent['userid'] ) ):
?>
	<!--<style>#armadilloSoloContent_6_stacks_in_12_937.draft { min-height: 2em; }</style>-->
	<?php if ( $soloContent == Armadillo_Language::public_msg($armadilloOptions, 'ARM_CONTENT_SELECTED_ITEM_MISSING') ): ?>
	<p style="margin: 20px 0;padding: 10px;border:solid 1px #ffcccc;color: #cc0000;"><?php echo Armadillo_Language::public_msg($armadilloOptions, 'ARM_SOLO_CONTENT_ID_MISSING'); ?></p>
	<!-- TODO: Add ajax call to create soloContent, refresh page if successfull or display error message otherwise -->
	<?php else: ?>
	<div class="armadilloEditButton"><a href="#editSoloContentID_6_stacks_in_12_937" class="displayEditor"><i class="fa fa-pencil-square-o"></i>&nbsp;ID: 6</a></div>
	<?php endif; ?>
	<div id="editSoloContentID_6_stacks_in_12_937" class="editSoloContent" style="display:none;">
		<div id="editSoloContentID_6_stacks_in_12_937_toolbar"></div>
		<?php if ( !isset($_COOKIE['armadillo']['editorType']) or ( isset($_COOKIE['armadillo']['editorType']) and ( $_COOKIE['armadillo']['editorType'] == 'richtext' or $_COOKIE['armadillo']['editorType'] == 'basictext' ) ) ): ?>
        <!-- Rich Text Editor -->
        <textarea id="soloContentID_6_stacks_in_12_937" name="soloContentID_6_stacks_in_12_937"><?php echo htmlspecialchars($soloContent['content'], ENT_QUOTES, 'UTF-8'); ?></textarea>
        <?php else: ?>
        <!-- Markdown Editor -->
        <textarea autofocus id="soloContentID_6_stacks_in_12_937" name="soloContentID_6_stacks_in_12_937" data-uk-htmleditor="{markdown:true,autofocus:true}"><?php if ( isset($soloContent['format']) and $soloContent['format'] == 'markdown' ) { echo $soloContent['rawContent']; } ?></textarea>
        <?php if ( !isset($soloContent['format']) or $soloContent['format'] == 'html' ): ?>
        <script>document.addEventListener("DOMContentLoaded", function(event) { soloContentID_6_stacks_in_12_937 = document.getElementById('soloContentID_6_stacks_in_12_937'); soloContentID_6_stacks_in_12_937.value = toMarkdown('<?php echo str_replace(array("\n", "\r"),'', str_replace("'","\'",$soloContent['rawContent'])); ?>'); });</script>
        <?php endif; ?>
	    <?php endif; ?>
		<p class="saveSoloContentDetails">
			<a href="#soloContentID_6_stacks_in_12_937" id="saveSoloContentID_6_stacks_in_12_937" class="saveSoloContentButton armadilloButton"><?php echo Armadillo_Language::public_msg($armadilloOptions, 'ARM_SOLO_CONTENT_SAVE_TEXT'); ?></a>
			<span class="saveSoloContent result"><i class="fa fa-lg"></i></span>
			<span class="saveSoloContent progress"></span>
		</p>
	</div>
	<script>
		document.addEventListener("DOMContentLoaded", function(event) {
			var armadilloSoloContent_6_stacks_in_12_937 = {};
			armadilloSoloContent_6_stacks_in_12_937 = (function(){
				<?php if ( !isset($_COOKIE['armadillo']['editorType']) or ( isset($_COOKIE['armadillo']['editorType']) and ( $_COOKIE['armadillo']['editorType'] == 'richtext' or $_COOKIE['armadillo']['editorType'] == 'basictext' ) ) ): ?>
				$R('#soloContentID_6_stacks_in_12_937', {
					toolbarExternal: '#editSoloContentID_6_stacks_in_12_937_toolbar',
					toolbarFixed: true,
					autoresize: false,
					maxHeight: 500,
					<?php if ( $_COOKIE['armadillo']['language'] !== 'default' ): ?>
				...
				<?php endif; ?>
				armasolo.jQuery('#saveSoloContentID_6_stacks_in_12_937').click(function(){
					<?php if ( !isset($_COOKIE['armadillo']['editorType']) or ( isset($_COOKIE['armadillo']['editorType']) and ( $_COOKIE['armadillo']['editorType'] == 'richtext' or $_COOKIE['armadillo']['editorType'] == 'basictext' ) ) ): ?>
					var updatedContent = $R('#soloContentID_6_stacks_in_12_937', 'source.getCode');
					var previewHTML = updatedContent;
					updatedContent = encodeURIComponent(updatedContent);
					var format = 'html';
					<?php else: ?>
					var updatedContent = encodeURIComponent(armasolo.jQuery('#soloContentID_6_stacks_in_12_937').val());
					var previewHTML = armasolo.jQuery('#editSoloContentID_6_stacks_in_12_937 .uk-htmleditor .uk-htmleditor-content .uk-htmleditor-preview').html();
					var format = 'markdown';
					<?php endif; ?>
					armasolo.jQuery.ajax({
						beforeSend: function(){ armasolo.jQuery('#editSoloContentID_6_stacks_in_12_937 .saveSoloContent.result i.fa').removeClass('fa-times, fa-check').parent().css('opacity',0); armasolo.jQuery('#editSoloContentID_6_stacks_in_12_937 .saveSoloContent.progress').fadeTo(300,1); },
						url: '../../rw_common/plugins/stacks/armadillo/core/model/updateArmadilloSettings.php',
						type: 'post',
						data: 'action=updateContent&format=' + format + '&contentID=6&content=' + updatedContent,
						success: function(data, status) {
							if ( data == 'true' ) {
								armasolo.jQuery('#editSoloContentID_6_stacks_in_12_937 .saveSoloContent.result i.fa').addClass('fa-check').parent().fadeTo(300,1);
								armasolo.jQuery('#armadilloSoloContent_6_stacks_in_12_937').html(previewHTML);
							} else { console.log(data); console.log(status); armasolo.jQuery('#editSoloContentID_6_stacks_in_12_937 .saveSoloContent.result i.fa').addClass('fa-times').parent().fadeTo(300,1); }
						},
						error: function(jqxhr, status, error) { console.log(error); console.log(jqxhr); armasolo.jQuery('#editSoloContentID_6_stacks_in_12_937 .saveSoloContent.result i.fa').addClass('fa-times').parent().fadeTo(300,1); },
						complete: function(){ armasolo.jQuery('#editSoloContentID_6_stacks_in_12_937 .saveSoloContent.progress').fadeTo(300,0); }
					});
				});
			})(armadilloSoloContent_6_stacks_in_12_937);
		});
	</script>
<?php endif; ?>
</div>

</div></div><div id='stacks_out_12_938' class='stacks_out'><div id='stacks_in_12_938' class='stacks_in com_nimblehost_stack_armadilloLoginLink_stack'>
