...
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
  ...
  <!-- Browser title -->
	<title>Festival Dates and Travel</title>
  ...
  <!-- Main Theme Stylesheet -->
	...
	<!-- Plugin injected code -->
		...
		<link rel='stylesheet' type='text/css' media='all' href='files/stacks_page_page10.css?rwcache=685984326' />
        <script type='text/javascript' charset='utf-8' src='https://ajax.googleapis.com/ajax/libs/jquery/2.2.4/jquery.min.js'></script>
        
        <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css">
        
		
		<script type='text/javascript' charset='utf-8' src='files/stacks_page_page10.js?rwcache=685984326'></script>
...
//include_once "files/stacks_page_page10.php";
...
<div class="text-xs-left">
	...
	<h2 class="theme_style  ">Festival Dates and Travel</h2>
...
if ( $setupIsComplete ) {
    ...
    $soloContent = displaySoloContent( $dbLink, '8', 'true', $armadilloOptions );
    ...
    if ( $soloContent == Armadillo_Language::public_msg($armadilloOptions, 'ARM_CONTENT_SELECTED_ITEM_MISSING') ) {
	    ...
	    if ( $soloContent['publish'] ) {
	    	$padding_for_8 = $soloContent['content'] == '' ? ' armadilloPadding' : '';
	    	echo '<div id="armadilloSoloContent_8_stacks_in_13" class="armadilloContent' . $padding_for_8 . '">' . $soloContent['content'] . '</div>';
	    } else {
	    	echo '<div id="armadilloSoloContent_8_stacks_in_13" class="armadilloContent draft"></div>';
    	}
    }
} else {
...
?>
	<!--<style>#armadilloSoloContent_8_stacks_in_13.draft { min-height: 2em; }</style>-->
	...
	<?php else: ?>
	<div class="armadilloEditButton"><a href="#editSoloContentID_8_stacks_in_13" class="displayEditor"><i class="fa fa-pencil-square-o"></i>&nbsp;ID: 8</a></div>
	<?php endif; ?>
	<div id="editSoloContentID_8_stacks_in_13" class="editSoloContent" style="display:none;">
		<div id="editSoloContentID_8_stacks_in_13_toolbar"></div>
        ...
        <!-- Rich Text Editor -->
        <textarea id="soloContentID_8_stacks_in_13" name="soloContentID_8_stacks_in_13"><?php echo htmlspecialchars($soloContent['content'], ENT_QUOTES, 'UTF-8'); ?></textarea>
        <?php else: ?>
        <!-- Markdown Editor -->
        <textarea autofocus id="soloContentID_8_stacks_in_13" name="soloContentID_8_stacks_in_13" data-uk-htmleditor="{markdown:true,autofocus:true}"><?php if ( isset($soloContent['format']) and $soloContent['format'] == 'markdown' ) { echo $soloContent['rawContent']; } ?></textarea>
        <?php if ( !isset($soloContent['format']) or $soloContent['format'] == 'html' ): ?>
        <script>document.addEventListener("DOMContentLoaded", function(event) { soloContentID_8_stacks_in_13 = document.getElementById('soloContentID_8_stacks_in_13'); soloContentID_8_stacks_in_13.value = toMarkdown('<?php echo str_replace(array("\n", "\r"),'', str_replace("'","\'",$soloContent['rawContent'])); ?>'); });</script>
        <?php endif; ?>
	    <?php endif; ?>
		<p class="saveSoloContentDetails">
			<a href="#soloContentID_8_stacks_in_13" id="saveSoloContentID_8_stacks_in_13" class="saveSoloContentButton armadilloButton"><?php echo Armadillo_Language::public_msg($armadilloOptions, 'ARM_SOLO_CONTENT_SAVE_TEXT'); ?></a>
	...
	<script>
		document.addEventListener("DOMContentLoaded", function(event) {
			var armadilloSoloContent_8_stacks_in_13 = {};
			armadilloSoloContent_8_stacks_in_13 = (function(){
				<?php if ( !isset($_COOKIE['armadillo']['editorType']) or ( isset($_COOKIE['armadillo']['editorType']) and ( $_COOKIE['armadillo']['editorType'] == 'richtext' or $_COOKIE['armadillo']['editorType'] == 'basictext' ) ) ): ?>
				$R('#soloContentID_8_stacks_in_13', {
					toolbarExternal: '#editSoloContentID_8_stacks_in_13_toolbar',
					toolbarFixed: true,
					autoresize: false,
					maxHeight: 500,
				...
				<?php endif; ?>
				armasolo.jQuery('#saveSoloContentID_8_stacks_in_13').click(function(){
					<?php if ( !isset($_COOKIE['armadillo']['editorType']) or ( isset($_COOKIE['armadillo']['editorType']) and ( $_COOKIE['armadillo']['editorType'] == 'richtext' or $_COOKIE['armadillo']['editorType'] == 'basictext' ) ) ): ?>
					var updatedContent = $R('#soloContentID_8_stacks_in_13', 'source.getCode');
					var previewHTML = updatedContent;
					updatedContent = encodeURIComponent(updatedContent);
					var format = 'html';
					<?php else: ?>
					var updatedContent = encodeURIComponent(armasolo.jQuery('#soloContentID_8_stacks_in_13').val());
					var previewHTML = armasolo.jQuery('#editSoloContentID_8_stacks_in_13 .uk-htmleditor .uk-htmleditor-content .uk-htmleditor-preview').html();
					var format = 'markdown';
					<?php endif; ?>
					armasolo.jQuery.ajax({
						beforeSend: function(){ armasolo.jQuery('#editSoloContentID_8_stacks_in_13 .saveSoloContent.result i.fa').removeClass('fa-times, fa-check').parent().css('opacity',0); armasolo.jQuery('#editSoloContentID_8_stacks_in_13 .saveSoloContent.progress').fadeTo(300,1); },
						url: '../../rw_common/plugins/stacks/armadillo/core/model/updateArmadilloSettings.php',
						type: 'post',
						data: 'action=updateContent&format=' + format + '&contentID=8&content=' + updatedContent,
						success: function(data, status) {
							if ( data == 'true' ) {
								armasolo.jQuery('#editSoloContentID_8_stacks_in_13 .saveSoloContent.result i.fa').addClass('fa-check').parent().fadeTo(300,1);
								armasolo.jQuery('#armadilloSoloContent_8_stacks_in_13').html(previewHTML);
							} else { console.log(data); console.log(status); armasolo.jQuery('#editSoloContentID_8_stacks_in_13 .saveSoloContent.result i.fa').addClass('fa-times').parent().fadeTo(300,1); }
						},
						error: function(jqxhr, status, error) { console.log(error); console.log(jqxhr); armasolo.jQuery('#editSoloContentID_8_stacks_in_13 .saveSoloContent.result i.fa').addClass('fa-times').parent().fadeTo(300,1); },
						complete: function(){ armasolo.jQuery('#editSoloContentID_8_stacks_in_13 .saveSoloContent.progress').fadeTo(300,0); }
					});
				});
			})(armadilloSoloContent_8_stacks_in_13);
		});
	</script>
<?php endif; ?>
</div>
