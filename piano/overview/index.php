...
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
  ...
  <!-- Main Theme Stylesheet -->
	...
	<!-- Plugin injected code -->
		...
		<link rel='stylesheet' type='text/css' media='all' href='files/stacks_page_page15.css?rwcache=685984326' />
        <script type='text/javascript' charset='utf-8' src='https://ajax.googleapis.com/ajax/libs/jquery/2.2.4/jquery.min.js'></script>
        
        <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css">
        
		
		<script type='text/javascript' charset='utf-8' src='files/stacks_page_page15.js?rwcache=685984326'></script>
...
//include_once "files/stacks_page_page15.php";
...
<div class="text-xs-left">
	...
	<h2 class="theme_style  ">Overview</h2>
...
if ( $setupIsComplete ) {
    ...
    $soloContent = displaySoloContent( $dbLink, '13', 'true', $armadilloOptions );
    ...
    if ( $soloContent == Armadillo_Language::public_msg($armadilloOptions, 'ARM_CONTENT_SELECTED_ITEM_MISSING') ) {
	    ...
	    if ( $soloContent['publish'] ) {
	    	$padding_for_13 = $soloContent['content'] == '' ? ' armadilloPadding' : '';
	    	echo '<div id="armadilloSoloContent_13_stacks_in_16" class="armadilloContent' . $padding_for_13 . '">' . $soloContent['content'] . '</div>';
	    } else {
	    	echo '<div id="armadilloSoloContent_13_stacks_in_16" class="armadilloContent draft"></div>';
    	}
    }
} else {
...
?>
	<!--<style>#armadilloSoloContent_13_stacks_in_16.draft { min-height: 2em; }</style>-->
	...
	<?php else: ?>
	<div class="armadilloEditButton"><a href="#editSoloContentID_13_stacks_in_16" class="displayEditor"><i class="fa fa-pencil-square-o"></i>&nbsp;ID: 13</a></div>
	<?php endif; ?>
	<div id="editSoloContentID_13_stacks_in_16" class="editSoloContent" style="display:none;">
		<div id="editSoloContentID_13_stacks_in_16_toolbar"></div>
        ...
        <!-- Rich Text Editor -->
        <textarea id="soloContentID_13_stacks_in_16" name="soloContentID_13_stacks_in_16"><?php echo htmlspecialchars($soloContent['content'], ENT_QUOTES, 'UTF-8'); ?></textarea>
        <?php else: ?>
        <!-- Markdown Editor -->
        <textarea autofocus id="soloContentID_13_stacks_in_16" name="soloContentID_13_stacks_in_16" data-uk-htmleditor="{markdown:true,autofocus:true}"><?php if ( isset($soloContent['format']) and $soloContent['format'] == 'markdown' ) { echo $soloContent['rawContent']; } ?></textarea>
        <?php if ( !isset($soloContent['format']) or $soloContent['format'] == 'html' ): ?>
        <script>document.addEventListener("DOMContentLoaded", function(event) { soloContentID_13_stacks_in_16 = document.getElementById('soloContentID_13_stacks_in_16'); soloContentID_13_stacks_in_16.value = toMarkdown('<?php echo str_replace(array("\n", "\r"),'', str_replace("'","\'",$soloContent['rawContent'])); ?>'); });</script>
        <?php endif; ?>
	    <?php endif; ?>
		<p class="saveSoloContentDetails">
			<a href="#soloContentID_13_stacks_in_16" id="saveSoloContentID_13_stacks_in_16" class="saveSoloContentButton armadilloButton"><?php echo Armadillo_Language::public_msg($armadilloOptions, 'ARM_SOLO_CONTENT_SAVE_TEXT'); ?></a>
	...
	<script>
		document.addEventListener("DOMContentLoaded", function(event) {
			var armadilloSoloContent_13_stacks_in_16 = {};
			armadilloSoloContent_13_stacks_in_16 = (function(){
				<?php if ( !isset($_COOKIE['armadillo']['editorType']) or ( isset($_COOKIE['armadillo']['editorType']) and ( $_COOKIE['armadillo']['editorType'] == 'richtext' or $_COOKIE['armadillo']['editorType'] == 'basictext' ) ) ): ?>
				$R('#soloContentID_13_stacks_in_16', {
					toolbarExternal: '#editSoloContentID_13_stacks_in_16_toolbar',
					toolbarFixed: true,
					autoresize: false,
					maxHeight: 500,
				...
				<?php endif; ?>
				armasolo.jQuery('#saveSoloContentID_13_stacks_in_16').click(function(){
					<?php if ( !isset($_COOKIE['armadillo']['editorType']) or ( isset($_COOKIE['armadillo']['editorType']) and ( $_COOKIE['armadillo']['editorType'] == 'richtext' or $_COOKIE['armadillo']['editorType'] == 'basictext' ) ) ): ?>
					var updatedContent = $R('#soloContentID_13_stacks_in_16', 'source.getCode');
					var previewHTML = updatedContent;
					updatedContent = encodeURIComponent(updatedContent);
					var format = 'html';
					<?php else: ?>
					var updatedContent = encodeURIComponent(armasolo.jQuery('#soloContentID_13_stacks_in_16').val());
					var previewHTML = armasolo.jQuery('#editSoloContentID_13_stacks_in_16 .uk-htmleditor .uk-htmleditor-content .uk-htmleditor-preview').html();
					var format = 'markdown';
					<?php endif; ?>
					armasolo.jQuery.ajax({
						beforeSend: function(){ armasolo.jQuery('#editSoloContentID_13_stacks_in_16 .saveSoloContent.result i.fa').removeClass('fa-times, fa-check').parent().css('opacity',0); armasolo.jQuery('#editSoloContentID_13_stacks_in_16 .saveSoloContent.progress').fadeTo(300,1); },
						url: '../../rw_common/plugins/stacks/armadillo/core/model/updateArmadilloSettings.php',
						type: 'post',
						data: 'action=updateContent&format=' + format + '&contentID=13&content=' + updatedContent,
						success: function(data, status) {
							if ( data == 'true' ) {
								armasolo.jQuery('#editSoloContentID_13_stacks_in_16 .saveSoloContent.result i.fa').addClass('fa-check').parent().fadeTo(300,1);
								armasolo.jQuery('#armadilloSoloContent_13_stacks_in_16').html(previewHTML);
							} else { console.log(data); console.log(status); armasolo.jQuery('#editSoloContentID_13_stacks_in_16 .saveSoloContent.result i.fa').addClass('fa-times').parent().fadeTo(300,1); }
						},
						error: function(jqxhr, status, error) { console.log(error); console.log(jqxhr); armasolo.jQuery('#editSoloContentID_13_stacks_in_16 .saveSoloContent.result i.fa').addClass('fa-times').parent().fadeTo(300,1); },
						complete: function(){ armasolo.jQuery('#editSoloContentID_13_stacks_in_16 .saveSoloContent.progress').fadeTo(300,0); }
					});
				});
			})(armadilloSoloContent_13_stacks_in_16);
		});
	</script>
<?php endif; ?>
</div>
